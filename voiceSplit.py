...
class Voicesplit(object):
    ...
    # wav文件写入,分时间间隔
    def wavWriteByTime(self, musicFileName, outData, voiceTime1, voiceTime2, sampleRate):
    ...
    def splitVoiceAndSave(self):
        ...
        for voiceTime in voiceTimeList:
            voiceTime1 = int(max(0, voiceTime[0]))
            voiceTime2 = int(min(sig.shape[0], voiceTime[-1]))
            self.wavWriteByTime(musicFileName=self.musicFileName, outData=inputData, voiceTime1=voiceTime1, voiceTime2=voiceTime2, sampleRate=self.sample_rate)


if __name__ == '__main__':
    # 在此输入你的音频文件
    musicFileName = './03.wav'
